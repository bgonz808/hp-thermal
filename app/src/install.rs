...
///
/// rstrtmgr.dll is DELAY-loaded (build.rs `/DELAYLOAD:rstrtmgr.dll`): it stays a *declared* import
/// (honest — the DLL and its functions live in the delay-import directory, fully visible to static
/// analysis) but the loader defers the actual load to this first call, which happens only on the
/// install/uninstall path — long after main()'s SetDefaultDllDirectories(SYSTEM32) pin. So rstrtmgr
/// and its non-KnownDLL dependency ncrypt resolve from System32, closing the pre-main DLL-hijack
/// window WITHOUT the dynamic-resolution (LoadLibrary+GetProcAddress) obfuscation. See #106.
